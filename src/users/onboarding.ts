  ...
  switch (user.status) {
    ...
    case "pending": {
      ...
      if (user.displayName === null) {
        ...
        updateDisplayName(db, user.phone, "");
      ...
      if (user.displayName === "") {
        ...
        updateDisplayName(db, user.phone, trimmedName);

        // Notify admin about the new user
        await messaging.send({
          to: config.ADMIN_PHONE as string,
          body: `New user request: ${trimmedName} (${user.phone}). Add their number to PHONE_WHITELIST to approve.`,
          from: config.TWILIO_PHONE_NUMBER,
        });

        log.info({ phone: user.phone, name: trimmedName }, "New user onboarding: admin notified");
